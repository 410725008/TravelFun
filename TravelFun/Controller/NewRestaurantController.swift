...
class NewRestaurantController: UITableViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    @IBAction func saveButtonTapped(sender: AnyObject) {
        ...
        if let appDelegate = (UIApplication.shared.delegate as? AppDelegate) {
            ...
            if restaurantToUpdate == nil { //add new data
                restaurant = TravelDataMO(context: context)
            ...
            restaurant.distric = typeTextField.text
            restaurant.address = addressTextField.text
            restaurant.tel = phoneTextField.text
            restaurant.introduction = descriptionTextView.text
            restaurant.image = "UpDaTeImAgEfRoMmObIlE"
            if let restaurantImage = photoImageView.image {
                restaurant.image2 = restaurantImage.pngData()
            }
    ...
    private func loadOldData() {
        ...
        typeTextField.text = restaurantToUpdate?.name
        addressTextField.text = restaurantToUpdate?.address
        phoneTextField.text = restaurantToUpdate?.tel
        descriptionTextView.text = restaurantToUpdate?.introduction
        if restaurantToUpdate?.image == "UpDaTeImAgEfRoMmObIlE"{
            if let restaurantImage = self.restaurantToUpdate?.image2 {
                photoImageView.image = UIImage(data: restaurantImage as Data)
            }
        }else if restaurantToUpdate?.image == "foodpin-logo"{
            photoImageView.image = UIImage(named: "foodpin-logo")
        }else if let imageAddress = restaurantToUpdate?.image {
            //產生url
                ...
                DispatchQueue.global().async {
                    do{
                        ...
                        DispatchQueue.main.async {
                            self.photoImageView.image = downLoadImage
                        }
                    }catch{
