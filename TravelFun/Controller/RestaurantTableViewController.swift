...
class RestaurantTableViewController: UITableViewController, UISearchResultsUpdating, NSFetchedResultsControllerDelegate {
    @IBOutlet var APItextlabel: UILabel!
    var traveldatas: [TravelDataMO] = []
    var fetchResultController: NSFetchedResultsController<TravelDataMO>!
    var restaurantToUpdate: TravelDataMO?
    var searchController: UISearchController!
    var searchResults: [TravelDataMO] = []
    ...
    override func viewDidLoad() {
        ...
        if traveldatas.isEmpty {
            print("generate data from begining")
            TravelData.writeTravelDataFromBegin()
        }
        APItextlabel.text = "點選按鈕連接至資料庫"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let restaurant = (searchController.isActive) ? searchResults[indexPath.row] : traveldatas[indexPath.row]
        
        
        cell.nameLabel.text = restaurant.name
        cell.locationLabel.text = restaurant.address
        cell.typeLabel.text = restaurant.distric
        if restaurant.image == "UpDaTeImAgEfRoMmObIlE"{
            if let restaurantImage = self.traveldatas[indexPath.row].image2 {
                cell.thumbnailImageView.image = UIImage(data: restaurantImage as Data)
            }
        }else if restaurant.image == "foodpin-logo"{
            cell.thumbnailImageView.image = UIImage(named: "foodpin-logo")
                ...
                DispatchQueue.global().async {
                    do{
                        ...
                        DispatchQueue.main.async {
                            cell.thumbnailImageView.image = downLoadImage
                        }
                    }catch{
        ...
        //if let restaurantImage = self.restaurants[indexPath.row].images {
        //    cell.thumbnailImageView.image = UIImage(data: restaurantImage as Data)
        //}
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let shareAction = UIContextualAction(style: .normal, title: "Share") { (action, sourceView, completionHandler) in
            let defaultText = "Just checking in at " + self.traveldatas[indexPath.row].name!
    ...
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let checkInAction = UIContextualAction(style: .normal, title: "Check-in") { (action, sourceView, completionHandler) in
            ...
            self.traveldatas[indexPath.row].isVisited = (self.traveldatas[indexPath.row].isVisited) ? false : true
            cell.accessoryType = self.traveldatas[indexPath.row].isVisited ? .checkmark : .none
            
            completionHandler(true)
        }
        
        // let checkInIcon = restaurants[indexPath.row].isVisited ? "arrow.uturn.left" : "checkmark"
        let checkInIcon = traveldatas[indexPath.row].isVisited ? "undo" : "tick"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showRestaurantDetail" {
            if let indexPath = tableView.indexPathForSelectedRow {
                ...
                destinationController.restaurant = (searchController.isActive) ? searchResults[indexPath.row] : traveldatas[indexPath.row]
            }
        }
    ...
    private func loadRestaurantFromDatabase() {
        let fetchRequest: NSFetchRequest<TravelDataMO> = TravelDataMO.fetchRequest()
    ...
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
        ...
        if let fetchedObjects = controller.fetchedObjects {
            traveldatas = fetchedObjects as! [TravelDataMO]
        }
    }
    ...
    func filterContent(for searchText: String) {
        ...
        searchResults = traveldatas.filter({ (restaurant) -> Bool in
    ...
    override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        let configuration = UIContextMenuConfiguration(identifier: indexPath.row as NSCopying, previewProvider: {
            ...
            let selectedRestaurant = self.traveldatas[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, willPerformPreviewActionForMenuWith configuration: UIContextMenuConfiguration, animator: UIContextMenuInteractionCommitAnimating) {
        ...
        let selectedRestaurant = self.traveldatas[selectedRow]
        restaurantDetailViewController.restaurant = selectedRestaurant
    ...
    @IBAction func btnGetClick(){
        if getPageCount > maxPage{
            APItextlabel.text = "加載完畢,共"+String(maxPage)+"頁"
            return
        }
        getTravelData()
        TravelData.AppendTravelData(sourceArray:traveldataget!)
        APItextlabel.text = "目前載入至第"+String(getPageCount)+"頁,共"+String(maxPage)+"頁"
    }
